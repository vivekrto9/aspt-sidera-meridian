...
test("repository docs describe Sidera Meridian without stale single-page or legacy catalog guidance", () => {
  const publicDocs = Object.values(docs).join("\n");
  const d1Schema = read("database/d1/001_initial_site_schema.sql");

  assert.match(docs.readme, /# Sidera Meridian/);
  ...
  assert.doesNotMatch(publicDocs, /templates\/sidera-meridian\/0\.1\.0\//);
...
test("lead documentation is an agent-ready integration reference", () => {
  ...
  assert.match(docs.leads, /wrangler d1 execute sidera-meridian-site --local/);
