...
This file is the implementation reference for agents deriving a theme from `sidera-meridian`. The base provides the generic D1 contract and server helpers, but intentionally does not expose a standalone public lead endpoint or ship vertical forms.
...
```sh
pnpm wrangler d1 execute sidera-meridian-site --local --command \
